...
export default async function CampaignAdminPage({
  ...
  try {
    const res = await fetch(`${appUrl}/api/campaigns/${campaignId}/participants`, {
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-muted text-muted-foreground uppercase text-xs">
                <tr>
                  ...
                  <th className="px-6 py-4 font-medium text-center">Humanity Verified</th>
                </tr>
              </thead>
              <tbody className="divide-y">
                {participants.length === 0 ? (
                  ...
                  participants.map((p) => (
                    <tr key={p.walletAddress} className="hover:bg-muted/50 transition-colors">
                      <td className="px-6 py-4 font-mono text-xs">
                        {p.walletAddress.slice(0, 6)}...{p.walletAddress.slice(-4)}
                      </td>
                      <td className="px-6 py-4">
                        {p.discordUsername ? (
                          <span className="font-medium text-indigo-500">@{p.discordUsername}</span>
                        ) : (
                      ...
                      <td className="px-6 py-4">
                        {p.telegramUsername ? (
                          <span className="font-medium text-sky-500">@{p.telegramUsername}</span>
                        ) : (
                          <span className="text-muted-foreground text-xs italic">Not Connected</span>
                        )}
                      </td>
